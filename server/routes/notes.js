...
router.delete('/deletenotepermanently/:id', fetchuser, async (req, res) => {
    try {
        ...
        if (!theNote) {
            return res.status(403).json({ error: "This interview doesn't exist" });
        }
...
router.put('/updatenote/:id', fetchuser, [
    ...
    try {
        ...
        const newNote = await NoteSchema.findByIdAndUpdate(req.params.id, { title: req.body.title,email: req.body.email, description: req.body.description });
